...
@Component({
  selector: 'app-company-offboarding',
  templateUrl: './company-offboarding.component.html',
  styleUrls: ['./company-offboarding.component.css']
})
export class CompanyOffboardingComponent implements OnInit {
  form: FormGroup;
  submitted = false;
  loading = false;
  ...
  ngOnInit(): void {
    this.route?.parent?.params.subscribe(params => {
      if (params && params.id) {
        ...
        this.getCompanyOffboardingChecklist();
      }
    });
  }


  getCompanyOffboardingChecklist() {
    this.companyOnboardOffboard.getCompanyOffboardingChecklist(this.companyId, 'company-checklist').subscribe(res => {
      this.companyOffboardData = res.data;
      if (!this.companyOffboardData.length) {
        this.rerender();
        return;
      }
      this.getFullyCompleteDate(this.companyOffboardData, true);
    });
  }
  ...
  async onSubmit() {
    ...
    let payload = {
      ...
      companyOffboardingId: this.selectedItem.id,
    ...
    || (this.selectedItem.completedDate != payload.completedDate)) {
      ...
      if (this.fullyCompletedStatus) {
        ...
        payload = { ...payload, ...{ companyOffboardCompletedDate: fullyCompletedDate, companyOffboardCompletedStatus: 1 } };
      } else {
        payload = { ...payload, ...{ companyOffboardCompletedDate: null, companyOffboardCompletedStatus: 0 } };
      }
    }

    this.companyOnboardOffboard.updateCompanyOffboardingChecklist(payload).subscribe(res => {
      if (!res) {
        return;
      }
  ...
  getChecklistStatusAndDate(payload: any): any{
    const checklistData = this.companyOffboardData;
