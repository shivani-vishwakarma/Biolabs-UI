...
import { CHECKLIST_STATUS, CONFIG, ROLE } from 'src/app/shared/utility/config.service';
import { environment } from 'src/environments/environment';
import { CompanyOnboardOffboardService } from '../../services/company-onboard-offboard.service';
...
export class CompanyOnboardingComponent implements OnInit {
  ...
  companyId: any;
  previousStatus: any;
  companyOnboardData: any = [];
  fullyCompletedDate: any = null;
  ...
  constructor(
    ...
    private companyOnboardOffboard: CompanyOnboardOffboardService,
  ...
  ngOnInit(): void {
    this.route?.parent?.params.subscribe(params => {
      if (params && params.id) {
        ...
        this.getCompanyOnboardingChecklist();
      }
    });
  }

  getCompanyOnboardingChecklist() {
    this.companyOnboardOffboard.getCompanyOnboardingChecklist(this.companyId, 'company-checklist').subscribe((res) => {
      this.companyOnboardData = res.data;
      if (!this.companyOnboardData.length) {
        this.rerender();
        return;
      }
      this.getFullyCompleteDate(this.companyOnboardData, true);
      // this.getChecklistStatusAndDate(true);
      // for (const element of this.companyOnboardData) {
  ...
  async onSubmit() {
    ...
    let payload: any = {
      ...
      key: 'company-checklist',
      companyOnboardingId: this.selectedItem.id,
    ...
    if ((this.selectedItem.checklist_status != payload.checklist_status)
      ...
      if (this.fullyCompletedStatus) {
        ...
        payload = { ...payload, ...{ companyOnboardCompletedDate: fullyCompletedDate, companyOnboardCompletedStatus: 1 } };
      } else {
        payload = { ...payload, ...{ companyOnboardCompletedDate: null, companyOnboardCompletedStatus: 0 } };
      }
    }
    this.companyOnboardOffboard.updateCompanyOnboardingChecklist(payload).subscribe(async (res) => {
      if (!res) {
        return;
      }
  ...
  getChecklistStatusAndDate(payload: any): any {
    const checklistData = this.companyOnboardData;
  ...
  onChangeStatus(event: any){
    for (const element of this.companyOnboardData) {
      if (element.checklist_status == 0 || element.checklist_status == 1) {
        this.fullyCompletedDate = null;
        this.fullyCompletedStatus = null;
        this.rerender();
        return;
      } else {
        if (element.completedDate && !this.fullyCompletedDate) {
