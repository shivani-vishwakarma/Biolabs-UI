...
@Component({
  selector: 'app-speditprofile',
  templateUrl: './speditprofile.component.html',
  styleUrls: ['./speditprofile.component.css']
})
export class SpeditprofileComponent implements OnInit {
  form: FormGroup;
  ...
  clickedOut(args: any) {
    ...
    this.defaultDataService.isNavTrue.subscribe((res: any) => {
      this.isNavTrue = res;
    });
    if ((id == 'logout' || id == 'mainSiteDropdown') && !this.isNavTrue) {
      const isChangeVal = (this.form.dirty && !this.isFormSubmitted)
        ...
        : true;
      if (id == 'logout') {
        // this.defaultDataService.isNavTrue = isChangeVal;
      }
      this.defaultDataService.isNavTrue.next(isChangeVal);
    }
  }
  ...
  constructor(
    ...
    this.form.valueChanges.subscribe(data => {
      if (this.form.dirty && !this.isFormSubmitted && defaultDataService.isNavTrue) {
        this.defaultDataService.isNavTrue.next(false);
      }
    });
  }
  ...
  getGlobalSponsorCompanyById(globalSponsorCompanyId: any) {
    this.globalSponsorService.getGlobalSponsorCompanyById(globalSponsorCompanyId).subscribe((res) => {
      const globalSponsorData = res.data;
  ...
  public canExit(isYes?: any): boolean {
    if (this.isNavTrue) {
      return true;
    }
  ...
  onSubmit() {
    ...
    this.globalSponsorService.addGlobalSponsor(data).subscribe((res) => {
      ...
      if (this.logoFileData) {
        ...
        this.defaultDataService.isNavTrue.next(true);
        this.router.navigate([`/spusertab/sptabprofile/${this.globalSponsorCompanyId}/spviewprofile`]);
      }
    }, (error: any) => {
