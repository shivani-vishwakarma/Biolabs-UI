...
@Component({
  selector: 'app-businessmanager-management',
  templateUrl: './businessmanager-management.component.html',
  styleUrls: ['./businessmanager-management.component.css']
})
export class BusinessmanagerManagementComponent implements OnInit {
  form: FormGroup;
  submitted = false;
  loading = false;
  config = {
  ...
  async onSubmit() {
    ...
    if (this.selectedData && this.selectedData.id) {
      ...
      response.then(data => {
        ...
        this.getUserList();
        this.loading = false;
        this.toastr.success(CONFIG.SUCCESS_MSG.UPDATE_BUSINESS_MANAGER, '', {
          timeOut: 3000,
          closeButton: true
        });
        ...
        this.loading = false;
        // this.modalService.dismissAll()
        this.toastr.error(error ? error.message : CONFIG.ERROR_MSG.UPDATE_BUSINESS_MANAGER, '', {
          timeOut: 3000,
          closeButton: true
        });
      });
      ...
      response.then(data => {
        ...
        this.getUserList();
        this.loading = false;
        this.toastr.success(CONFIG.SUCCESS_MSG.ADD_BUSINESS_MANAGER, '', {
          timeOut: 3000,
          closeButton: true
        });
        this.modalService.dismissAll();
        ...
        this.loading = false;
        // this.modalService.dismissAll()
        this.toastr.error(error ? error.message : CONFIG.ERROR_MSG.ADD_BUSINESS_MANAGER, '', {
          timeOut: 3000,
          closeButton: true
        });
      });
  ...
  getUserList() {
    this.spinner.show();
    const userList = this.userMgmtService.getUserList('Business_Manager');
    userList.subscribe(resp => {
      ...
      this.spinner.hide();
      this.loading = false;
      this.toastr.error(error ? error.message : CONFIG.ERROR_MSG.LIST_BUSINESS_MANAGER, '', {
        timeOut: 3000,
        closeButton: true
      });
      this.spinner.hide();
  ...
  async deleteUserData(): Promise<any> {
    ...
    response.then(data => {
      this.getUserList();
      this.loading = false;
      this.toastr.success(CONFIG.SUCCESS_MSG.DELETE_BUSINESS_MANAGER, '', {
        timeOut: 3000,
        closeButton: true
      });
      this.modalService.dismissAll();
      ...
      this.getUserList();
      this.loading = false;
      this.toastr.error(error ? error.message : CONFIG.ERROR_MSG.DELETE_BUSINESS_MANAGER, '', {
        timeOut: 3000,
        closeButton: true
      });
    });
