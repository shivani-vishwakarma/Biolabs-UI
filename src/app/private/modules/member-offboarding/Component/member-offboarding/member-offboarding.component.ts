...
export class MemberOffboardingComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route?.parent?.params.subscribe(params => {
      if (params && params.id) {
        ...
        this.getMemberOffboardingChecklist();
      }
    });
  }
  getMemberOffboardingChecklist() {
    this.memberOnboardOffboard.getMemberOffboardingChecklist(this.memberId, 'member-checklist').subscribe((res: any) => {
      this.memberOffboardData = res.data;
      if (!this.memberOffboardData.length) {
        this.rerender();
        return;
      }
      this.getFullyCompleteDate(this.memberOffboardData, true);
    });
  }
  ...
  async onSubmit() {
    ...
    let payload = {
      ...
      memberOffboardingId: this.selectedItem.id,
    ...
    || (this.selectedItem.completedDate != payload.completedDate)) {
      ...
      if (this.fullyCompletedStatus) {
        ...
        payload = { ...payload, ...{ memberOffboardCompletedDate: fullyCompletedDate, memberOffboardCompletedStatus: 1 } };
      } else {
        payload = { ...payload, ...{ memberOffboardCompletedDate: null, memberOffboardCompletedStatus: 0 } };
      }
    }
    this.memberOnboardOffboard.updateMemberOffboardingChecklist(payload).subscribe((res: any) => {
      if (!res) {
        return;
      }
  ...
  getChecklistStatusAndDate(payload: any): any{
    const checklistData = this.memberOffboardData;
