...
export class InvoiceSummaryComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private async getSiteData() {
    ...
    this.siteData = sites.filter((site: any) => site.id == this.siteId)[0];
    this.initiateDataTable();
  }

  initiateDataTable() {
    const siteName = (this.siteData && this.siteData.name) ? this.siteData.name : 'Biolabs';
  ...
  redirectToInvoicing(companyId: number , data: any) {
    localStorage.setItem('companyDet', JSON.stringify(data));
    this.router.navigate([`resident-companies/${companyId}/invoicing`]);
    // this.router.navigate([`resident-companies/${companyId}/invoicing`], { queryParams: { data: JSON.stringify(data) } });
  }
  ...
  private getInvoiceData(month: number, year: number) {
    ...
    this.getSiteData();
    this.invoiceService.getALLInvoiceByMonth(month, year)
      .subscribe(
        (response) => {
          this.data = [];
          this.summaryTotal = 0;
          ...
          for (const [i, orderProduct] of response.entries()) {
            ...
            const companyJson = this.resetTotalJson();
            ...
            if (companyNameArr.indexOf(orderProduct.companyName) == -1) {
              ...
              if (i > 0) {
                 ...
                 * JSON for Total Row
                 */
                const totalJson = this.resetTotalJson();
                totalJson.total = total;
                this.summaryTotal = this.summaryTotal + total;
                this.data.push(totalJson);
                total = 0;
              }
              /**
